...
export default function DynamicContentBrowser({ gradeLevel }: { gradeLevel: string }) {
    ...
    // 2. Charger le contenu depuis l'API Bunny backend quand le type change
    useEffect(() => {
        ...
        async function fetchContent() {
            ...
            try {
                ...
                if (selectedType === "videos") {
                    if (mapping.bunnyCollectionId) {
                        url = `/api/user/content/bunny?collectionId=${mapping.bunnyCollectionId}&t=${Date.now()}`;
                    } else {
                        setItems([]);
                        return;
                    }
                } else {
                    if (mapping.bunnyStoragePath) {
                        url = `/api/user/content/bunny?path=${encodeURIComponent(mapping.bunnyStoragePath)}&t=${Date.now()}`;
                    } else {
                        setItems([]);
                        return;
                    }
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {["Mathématiques", "Informatique"].map((subject, idx) => (
                        ...
                        >
                            ...
                            <h3 className="text-2xl font-black text-gray-900 mb-2">{subject}</h3>
                            <p className="text-gray-500 font-medium relative z-10">Vidéos et documents HD anti-téléchargement.</p>
                            <div className="mt-8 flex items-center gap-2 text-brand-orange font-bold">
                                Parcourir les cours <ChevronRight className="w-4 h-4 group-hover:translate-x-2 transition-transform duration-300" />
                            </div>
                        </button>
                    ))}
                </div>
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-700 bg-slate-50/60 p-6 md:p-8 rounded-[3rem] border border-slate-100/80 shadow-inner">
            ...
            <div className="flex flex-col md:flex-row gap-6 items-start">
                ...
                <div className="w-full md:w-72 shrink-0 flex flex-col gap-2 p-4 bg-white/80 backdrop-blur-md rounded-[2rem] border border-white shadow-lg shadow-slate-200/50">
                    <h3 className="px-4 py-2 text-xs font-black text-slate-400 uppercase tracking-wider mb-2">Ressources HQ</h3>
                    {[
                ...
                <div className="flex-1 w-full min-h-[400px]">
                    {itemsLoading ? (
                        ...
                        <div className="text-center py-24 bg-white/60 backdrop-blur border-2 border-dashed border-slate-200 rounded-[2.5rem] animate-in zoom-in-95 duration-500">
                            ...
                            <p className="text-slate-500 font-bold text-lg">Aucun contenu trouvé sur Bunny.net</p>
                            <p className="text-slate-400 text-sm mt-1">Assurez-vous que le dossier ou la collection est lié via l'Admin.</p>
                        </div>
                    ) : items[0]?.isError ? (
                        <div className="col-span-full text-center py-24 bg-red-50/50 backdrop-blur border-2 border-dashed border-red-200 rounded-[2.5rem] animate-in zoom-in-95 duration-500">
                            <p className="text-red-600 font-bold text-xl mb-2">Erreur Bunny API</p>
                            <p className="text-red-500 font-medium">{items[0].message}</p>
                        </div>
                    ) : (
                                ...
                                items.map((video: any, idx: number) => (
                                    ...
                                    >
                                        ...
                                        <div className="p-5 bg-white text-left">
                                            <h4 className="font-bold text-slate-800 line-clamp-2 group-hover:text-brand-orange transition-colors">{video.name}</h4>
                                        </div>
                                    </button>
                                ))
                            ) : (
                                items.map((file: any, idx: number) => (
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-[11px] text-slate-400 uppercase font-black tracking-wider mt-0.5">Format PDF</p>
                                        </div>
                                    </button>
                                ))
                            )}
