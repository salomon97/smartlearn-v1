...
async function main() {
    if (!fs.existsSync(CREDENTIALS_PATH)) {
        ...
        console.log("Veuillez créer des identifiants OAuth 2.0 (Application de bureau) sur Google Cloud Console et les enregistrer sous 'credentials.json'.");
        process.exit(1);
    }
    ...
    // Récupérer les playlists existantes pour éviter les doublons
    console.log("🔍 Récupération des playlists existantes...");
    const existingPlaylists: { [title: string]: string } = {};
    ...
    // Save results to a temporary CSV for easy import
